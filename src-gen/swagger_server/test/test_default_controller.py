...
class TestDefaultController(BaseTestCase):
    ...
    def test_indexers_indexer_type_data_sources_data_source_id_records_get(self):
        ...
        """
        response = self.client.open(
            '/indexers/{indexerType}/dataSources/{dataSourceId}/records'.format(indexer_type='indexer_type_example', data_source_id='data_source_id_example'),
            method='GET')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_indexers_indexer_type_data_sources_get(self):
        ...
        """
        response = self.client.open(
            '/indexers/{indexerType}/dataSources'.format(indexer_type='indexer_type_example'),
            method='GET')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_jobs_job_id_delete(self):
        ...
        """
        response = self.client.open(
            '/jobs/{jobId}'.format(job_id='job_id_example'),
            method='DELETE')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_jobs_job_id_request_logs_update_post(self):
        """Test case for jobs_job_id_request_logs_update_post

        Requests an update to the logs for a specific job without providing a request body.
        """
        response = self.client.open(
            '/jobs/{jobId}/request-logs-update'.format(job_id='job_id_example'),
            method='POST')
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
